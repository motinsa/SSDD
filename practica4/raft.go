...
type NodoRaft struct {
	...
	Nodos []rpctimeout.HostPort // Conexiones RPC a todos los nodos (réplicas) Raft
...
func NuevoNodo(nodos []rpctimeout.HostPort, yo int, canalAplicar chan AplicaOperacion) *NodoRaft {
	nr := &NodoRaft{}
	nr.Nodos = nodos
	nr.yo = yo
...
func (nr *NodoRaft) enviarPeticionVoto(nodo int, args *ArgsPeticionVoto,
	...
	ok := false
	client, err := rpc.DialHTTP("tcp", rpctimeout.HostPortArrayToString(nr.Nodos[nodo]))

	if err == nil {
		err = rpctimeout.CallTimeout(client, "NodoRaft.PedirVoto", args, reply, 100*time.Millisecond)
...
func (nr *NodoRaft) gestionLider() {
	...
	tiempoElecciones := 300 // Igual meter esto en el struct del Nodo
...
func (nr *NodoRaft) AppendEntries(args *ArgsAppend, reply *RespuestaAppend) error {
	...
	if args.Term > nr.currentTerm {
		nr.logger.Println("Mandato menor en AppendEntries")
		nr.estado = "Follower"
	}
	reply.Success = false
	if args.Term == nr.currentTerm {
	...
	reply.Term = nr.currentTerm
	nr.logger.Println("Respuesta Append: +%+v", *reply)
	return nil
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	nr.logger.Println("AppendEntries: %+v", args)
